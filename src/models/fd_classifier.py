...
def plot_confusion_matrix(y_true, y_pred, title, filename):
    FD_MODEL_DIR.mkdir(parents=True, exist_ok=True)
    cm = confusion_matrix(y_true, y_pred)
    plt.figure(figsize=(8, 6))
    sns.heatmap(cm, annot=True, fmt='d', cmap='Greens') # Changed color to distinguish
    plt.title(title)
    plt.ylabel('True Label')
    plt.xlabel('Predicted Label')
    output_path = FD_MODEL_DIR / filename
...
def main():
    ...
    print("\n" + "="*40)
    print("FINAL RESULTS SUMMARY (SET B: Classic + Fractal)")
    print("="*40)
