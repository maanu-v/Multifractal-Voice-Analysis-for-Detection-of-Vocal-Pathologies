...
from src.utils.config import FD_MFDFA_FEATURES_CSV, FD_MFDFA_MODEL_DIR

logger = get_logger(__name__)

def plot_confusion_matrix(y_true, y_pred, title, filename):
    FD_MFDFA_MODEL_DIR.mkdir(parents=True, exist_ok=True)
    ...
    output_path = FD_MFDFA_MODEL_DIR / filename
...
def train_and_evaluate(name, pipeline, param_grid, X_train, y_train, X_test, y_test):
    ...
    plot_confusion_matrix(
        y_test, 
        y_pred_tuned, 
        f"{name} Confusion Matrix (Fractal Only)", 
...
def main():
    if not FD_MFDFA_FEATURES_CSV.exists():
        logger.error(f"Data file not found: {FD_MFDFA_FEATURES_CSV}")
        return

    # Load Data (Fractal Only)
    logger.info(f"Loading data from {FD_MFDFA_FEATURES_CSV}...")
    df = pd.read_csv(FD_MFDFA_FEATURES_CSV)
    ...
    print("\n" + "="*50)
    print("FINAL RESULTS SUMMARY (Fractal Features Only: Higuchi + Wavelet + MF-DFA)")
    print("="*50)
