...
from src.utils.config import MFDFA_FEATURES_CSV, MULTIFRACTAL_MODEL_DIR

logger = get_logger(__name__)

def plot_confusion_matrix(y_true, y_pred, title, filename):
    MULTIFRACTAL_MODEL_DIR.mkdir(parents=True, exist_ok=True)
    ...
    output_path = MULTIFRACTAL_MODEL_DIR / filename
...
def train_and_evaluate(name, pipeline, param_grid, X_train, y_train, X_test, y_test):
    ...
    plot_confusion_matrix(
        y_test, 
        y_pred_tuned, 
        f"{name} Confusion Matrix (MF-DFA Only)", 
        f"cm_mfdfa_only_{name.lower().replace(' ', '_').replace('(', '').replace(')', '')}.png"
    )
...
def main():
    if not MFDFA_FEATURES_CSV.exists():
        logger.error(f"Data file not found: {MFDFA_FEATURES_CSV}")
        return

    # Load Data (MF-DFA Only)
    logger.info(f"Loading data from {MFDFA_FEATURES_CSV}...")
    df = pd.read_csv(MFDFA_FEATURES_CSV)
    ...
    print("\n" + "="*50)
    print("FINAL RESULTS SUMMARY (MF-DFA Features Only)")
    print("="*50)
