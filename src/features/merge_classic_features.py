...
from src.utils.config import PRAAT_FEATURES_CSV, MFCC_FEATURES_CSV, CLASSIC_FEATURES_CSV
...
def main():
    ...
    if not PRAAT_FEATURES_CSV.exists():
        logger.error(f"Baseline features not found at {PRAAT_FEATURES_CSV}")
        return
    ...
    baseline = pd.read_csv(PRAAT_FEATURES_CSV)
    ...
    CLASSIC_FEATURES_CSV.parent.mkdir(parents=True, exist_ok=True)
